...
struct Obstacles {
    int state; //0-zużyte, 1-mąka, 2-widły, 3-owczy defragmentator, 4-owcze fragmenty w lewo, 5-owcze fragmenty w prawo
    float x1;
    float y1;
    float a;
    float b; //'p' dla paraboli defragmentatora
    float q;
    float x3;
    float y3;
    int i; //1 lub -1 w zależności, czy defragment ma lecieć w prawo, czy w lewo
    float frame_y;
...
struct Fodder {
    ...
    bool launch; //czy pastuch strzelił owcą? (jeden pastuch ma jedną owcę)
    float x1;
    float y1;
    float x2;
    float y2;
...
struct Play { //struktura zawierająca zmienne wykorzystywane głównie podczas trwania gry
    ...
    ALLEGRO_SAMPLE *psong, *flap1, *flap2, *flap3, *flap4, *die, *hit1, *hit2, *whoosh1, *whoosh2, *lfire, *sheep, *hiss;
    ALLEGRO_SAMPLE_INSTANCE *in_psong, *in_flap1, *in_flap2, *in_flap3, *in_flap4, *in_die, *in_hit1, *in_hit2, *in_whoosh1, *in_whoosh2, *in_lfire, *in_sheep, *in_hiss;
...
struct Menu { //struktura zawierająca zmienne używane przez menu gry
...
struct Game { //główna struktura gry przekazywana do wszystkich funkcji w programie
